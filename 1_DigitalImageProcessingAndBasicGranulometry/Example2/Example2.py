...
#cut out printout
img1 = img1[50:-10,10:]
cv2.imwrite("output/Example2Cut.png", img1)
...
plt.title('Hist Red : ')
plt.savefig("output/Example2ColorHistograms")
plt.show()
...
cv2.imwrite("output/Example2Equalization.png", img1)
...
plt.plot(hist3/max(hist3),color="red")   
plt.title('Hist Red : ')
plt.savefig("output/Example2ColorHistogramsEqualization.png")
plt.show()
...
cv2.imwrite("output/Example2BWThresholding.png", thresh1)
...
cv2.imwrite("output/Example2BWCountour.png", blank)
...
cv2.imwrite("output/Example2BWMinAreaRectangle.png", img1_tmp1)
cv2.imwrite("output/Example2BWMinAreaRectangleWithIndices.png", img1_tmp2)
...
plt.ylabel('WSSE')
plt.grid(True)
plt.savefig("output/Example2kmeans.png")
plt.show()
...
for label, df_grouped in df.groupby('label'):
    ...
    cv2.imwrite("output/Example2label{}.png".format(label), masked_image)
    ...
    for  idx in df_grouped.index:
        ...
        if max(ellipse[1]) < 50:
            ...
            cv2.ellipse(img_ellipse_closed, ellipse, (0,0,label*50), -1, cv2.LINE_AA) #open ellypses
...
cv2.imwrite('output/Example2_color_segmentation.png', img_tmp)
cv2.imwrite('output/Example2_color_ellypses.png', img_ellipse)
cv2.imwrite('output/Example2_color_ellypses_closed.png', img_ellipse_closed)

#angle histogram
plt.hist(np.array(df_ellipses.angle)%90)
plt.xlabel("angle[deg]  mod 90")
plt.ylabel("No. of ellipses")
plt.savefig("output/Example2AngleDistibution.png")
plt.show()
